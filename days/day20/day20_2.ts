...
let buttonPresses = 0;
let inputCycle: [number][] = [[], [], [], []];

for (let i = 0; i < 100_000; i++) pushButton();

console.log(inputCycle);
console.log(
	BigInt(inputCycle[0][0]) *
		BigInt(inputCycle[1][0]) *
		BigInt(inputCycle[2][0]) *
		BigInt(inputCycle[3][0])
);
...
function runPulses(pulses: Pulse[]): Pulse[] {
	...
	pulses.forEach((p) => {
		...
		if (mod.type === "%") {
			...
			if (m2.targets.includes("rx")) {
				if (m2.remember[rmb][1] === "high") {
					inputCycle[rmb].push(buttonPresses);
				}
			}
		} else {
